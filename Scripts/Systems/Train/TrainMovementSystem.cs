...
public class TrainMovementSystem(Label speedLabel) : ISystem {
    ...
    public void Update(World world, double delta) {
        Option<Entity> entityOpt = world.QueryFirst<TrainMovementComponent>();
    ...
    private static void UpdatePhysics(ref TrainMovementComponent movement, float delta) {
        ...
        float targetSpeed = movement.IsBraking ? 0f : movement.MaxSpeed;
        float accelRate = movement.IsBraking ? movement.Deceleration : movement.Acceleration;
    ...
    private void UpdateHud(TrainMovementComponent movement) {
        string distanceStr = $"{movement.DistanceTraveled / 1000f:F1} km";
        speedLabel.Text = movement.IsBraking
            ? $"▸ {movement.Speed:F0} u/s   {distanceStr}   ▌▌ BRAKE"
            : $"▸ {movement.Speed:F0} u/s   {distanceStr}";
    }
}
